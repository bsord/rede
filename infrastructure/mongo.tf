...
resource "aws_ssm_parameter" "primary_db_connection_string" {
  ...
  type = "SecureString"
  value = "${replace(mongodbatlas_cluster.atlas_cluster.connection_strings[0].standard_srv, "mongodb+srv://", "mongodb+srv://${mongodbatlas_database_user.db-user.username}:${coalesce(nonsensitive(mongodbatlas_database_user.db-user.password), "null")}@")}/${var.mongodb_atlas_project_name}-db"
